...
- **运行依赖**：PostgreSQL 15 + TimescaleDB 2.x（本地原生安装；DSN 见 `config.example.yaml`）
...
> 前置：本机已装 PostgreSQL 15 + TimescaleDB 2.x，DSN 与 `config.example.yaml` 默认值一致（或通过 `FUNDPILOT_DATABASE_DSN` 覆盖）。

```bash
# 1. 准备配置（可选；默认配置就足够本地跑）
cd ..
